...
sim_reg_stats(struct stat_sdb_t *sdb)
{
  ...
  stat_reg_counter(sdb, "sim_num_RAW_stalls_q1", "total number of stalls due to RAW hazards (q1)",
		   &sim_num_RAW_stalls_q1, sim_num_RAW_stalls_q1, NULL);

  stat_reg_counter(sdb, "sim_num_RAW_stalls_q2", "total number of stalls due to RAW hazards (q2)",
		   &sim_num_RAW_stalls_q2, sim_num_RAW_stalls_q2, NULL); 

  stat_reg_counter(sdb, "cur_cycle_q1", "the current clock cycle of execution", &cur_cycle_q1, cur_cycle_q1, NULL);

  stat_reg_counter(sdb, "cur_cycle_q2", "the current clock cycle of execution", &cur_cycle_q2, cur_cycle_q2, NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q1",
		   "CPI from RAW hazard (q1)",
		   "cur_cycle_q1/sim_num_insn" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);

  stat_reg_formula(sdb, "CPI_from_RAW_hazard_q2",
		   "CPI from RAW hazard (q2)",
		   "cur_cycle_q2/sim_num_insn" /* ECE552 - MUST ADD YOUR FORMULA */, NULL);
...
/* ECE552 Assignment 0 - BEGIN CODE 	
 ...
 ECE552 Assignment 0 - END CODE */
/* ECE552 Assignment 1 - BEGIN CODE */
      // need to count stalls separately from hazards, also keep track of current cycle
	cur_cycle_q1++; // add these to top
	cur_cycle_q2++;
	int i;
	// check for hazards
	int hazard_q1 = 0;
	...
	for (i=0;i<3;i++) {
		...
		//}	
		if (r_in[i] != DNA){
		       if (reg_ready_q1[r_in[i]] > cur_cycle_q1){
				hazard_q1 = 1;
				// figure out how many stalls to add
				if (reg_ready_q1[r_in[i]] - cur_cycle_q1 > num_stalls_q1){
			      		num_stalls_q1 = reg_ready_q1[r_in[i]] - cur_cycle_q1;
				}
			}
			if (reg_ready_q2[r_in[i]] > cur_cycle_q2){
				if ((i==0) && (MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_STORE)){
					continue;
				}	
				hazard_q2 = 1;
				if (reg_ready_q2[r_in[i]] - cur_cycle_q2 > num_stalls_q2){
					num_stalls_q2 = reg_ready_q2[r_in[i]] - cur_cycle_q2;
				}	
			}
		}	
	}
	if (hazard_q1 > 0){
		...
		sim_num_RAW_stalls_q1 += num_stalls_q1;
		cur_cycle_q1 += num_stalls_q1;
	}
	if (hazard_q2 > 0) {
		sim_num_RAW_hazard_q2++;
		sim_num_RAW_stalls_q2 += num_stalls_q2;
		cur_cycle_q2 += num_stalls_q2;
	}	
   	if ((MD_OP_FLAGS(op) & F_MEM) && (MD_OP_FLAGS(op) & F_LOAD)|| (MD_OP_FLAGS(op) & F_ICOMP) || (MD_OP_FLAGS(op) & F_FCOMP)) {
		for (int i=0;i<2;i++){
			if (r_out[i] != DNA){
				reg_ready_q1[r_out[i]] = cur_cycle_q1 + 3;
				if ((MD_OP_FLAGS(op) & F_ICOMP) || (MD_OP_FLAGS(op) & F_FCOMP)){
					reg_ready_q2[r_out[i]] = cur_cycle_q2 + 2;
				}
				else {
					reg_ready_q2[r_out[i]] = cur_cycle_q2 + 3;
				}	
			}
		}
	}
